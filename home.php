                ...
                <div class="card">
                    <form method="GET" style="display:flex; flex-wrap:wrap; gap:15px; align-items:center;">
                        ...
                        <div style="flex:1; min-width: 200px;">
                            ...
                            <div id="hist-input-monthly" class="hist-input" style="display:<?php echo ($hist_mode=='monthly')?'flex':'none'; ?>; gap:10px;"><select name="m" class="date-control">
            <?php  $thai_months = [1=>'มกราคม', 2=>'กุมภาพันธ์', 3=>'มีนาคม', 4=>'เมษายน', 5=>'พฤษภาคม', 6=>'มิถุนายน', 7=>'กรกฎาคม', 8=>'สิงหาคม', 9=>'กันยายน', 10=>'ตุลาคม', 11=>'พฤศจิกายน', 12=>'ธันวาคม' ];for($i=1; $i<=12; $i++) {$selected = ($i == ($_GET['m'] ?? date('n'))) ? 'selected' : '';echo "<option value='$i' $selected>{$thai_months[$i]}</option>";}?>
        </select><select name="y" class="date-control" style="flex:1"><?php for($y=date('Y');$y>=date('Y')-2;$y--) echo "<option value='$y' ".($y==($_GET['y']??date('Y'))?'selected':'').">".($y+543)."</option>"; ?></select></div>
                            <div id="hist-input-quarterly" class="hist-input" style="display:<?php echo ($hist_mode=='quarterly')?'flex':'none'; ?>; width:100%; gap:10px;"><select name="q_q" class="date-control" style="flex:1;"><option value="1" <?php echo (($_GET['q_q'] ?? '') == '1') ? 'selected' : ''; ?>>Q1 (ไตรมาส 1)</option><option value="2" <?php echo (($_GET['q_q'] ?? '') == '2') ? 'selected' : ''; ?>>Q2 (ไตรมาส 2)</option><option value="3" <?php echo (($_GET['q_q'] ?? '') == '3') ? 'selected' : ''; ?>>Q3 (ไตรมาส 3)</option><option value="4" <?php echo (($_GET['q_q'] ?? '') == '4') ? 'selected' : ''; ?>>Q4 (ไตรมาส 4)</option></select><select name="q_y" class="date-control" style="flex:1;"><?php for($y=date('Y'); $y>=date('Y')-5; $y--) {$selected = ($y == ($_GET['q_y'] ?? date('Y'))) ? 'selected' : '';echo "<option value='$y' $selected>" . ($y + 543) . "</option>"; }?></select></div>
                            <div id="hist-input-yearly" class="hist-input" style="display:<?php echo ($hist_mode=='yearly')?'flex':'none'; ?>;"><select name="y_y" class="date-control" style="width:100%"><?php for($y=date('Y'); $y>=date('Y')-5; $y--) {$selected = ($y == ($_GET['y_y'] ?? date('Y'))) ? 'selected' : '';echo "<option value='$y' $selected>" . ($y + 543) . "</option>";}?></select></div>
                        </div>
